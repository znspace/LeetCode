...
const { PROBLEMS_PATH, README_BASE_PATH, README_PATH } = require('./config');
const { DIFFICULTY_MAP, getQuestionsDetails, formatId, questionUrl } = require('./script-utils');

const getSolvedQuestionIds = () =>
  new Promise(resolve => {
...
const getQuestionsTableContent = questions =>
  questions.reduce(
    ([items, links], question) => {
...
getSolvedQuestionIds()
  .then(ids => getQuestionInfos(ids), err => Promise.reject(err))
  .then(questions => regenerateReadme(questions), err => Promise.reject(err));
